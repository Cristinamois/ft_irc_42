...
void RFC1459_PRIVMSG(Client* sender, parsed_cmd_t& pc)
{
    ...
    if (!pc.has_words)
        srv->sendString(sender, format(ERR_NORECIPIENT, args));
    else if (!pc.has_suffix)
        srv->sendString(sender, format(ERR_NOTEXTTOSEND, args));
    else
    {
        ...
        {
            args.push_back(dup);
            srv->sendString(sender, format(ERR_TOOMANYTARGETS, args));
            return;
        }
        ...
        for (i = 0; i < recipients.size(); ++i)
        {
            if (recipients[i][0] == '&' || recipients[i][0] == '#')
            {
                ...
                if (!chan || (chan->getMode()->n && !chan->isConnected(sender)))
                {
                    ...
                    srv->sendString(sender, format(ERR_CANNOTSENDTOCHAN, args));
                    continue;
                }
            ...
            else
            {
                ...
                args.push_back(recipients[i]);
                args.push_back(pc.suffix);
                srv->sendString(cli, format(RPL_PRIVMSG, args));
            }
        }
    }
}
